...
module.exports = {
  ...
  options: [{
    name: "number",
    description: "kaç şarkıyı atlamak istediğinizi belirtin",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (number) {
        if (!queue.songs.length > number) return interaction.reply({ content: '⚠️ Mevcut şarkı sayısı aşıldı', ephemeral: true }).catch(e => { })
        if (isNaN(number)) return interaction.reply({ content: '⚠️ Geçersiz numara', ephemeral: true }).catch(e => { })
        if (1 > number) return interaction.reply({ content: '⚠️ Geçersiz numara', ephemeral: true }).catch(e => { })
...
try {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(success ? ` **ATLANDI** : **${old.name}**` : '❌ Sıra boş!')
    .setTimestamp();
