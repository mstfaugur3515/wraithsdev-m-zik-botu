...
module.exports = {
  name: "play",
  description: "gelin biraz müzik dinleyelim!!",
  ...
  run: async (client, interaction) => {
    try {
      ...
      try {
        ...
        return interaction.editReply({ content: `❌ Sonuç yok` }).catch(e => { });
      }

      if (!res || !res.length || !res.length > 1) return interaction.reply({ content: `❌ Sonuç yok`, ephemeral: true }).catch(e => { });

      const embed = new EmbedBuilder();
      embed.setColor(client.config.embedColor);
      embed.setTitle(`Müzik Bulundu: ${name}`);
      ...
      interaction.reply(code).then(async Message => {
        ...
        collector.on('collect', async (button) => {
          switch (button.customId) {
            case 'cancel': {
              embed.setDescription(`Arama kesintiye uğradı`);
            ...
            default: {
              ...
              try {
                ...
                await interaction.editReply({ content: `❌ Sonuç Yok!`, ephemeral: true }).catch(e => { });
              }
              return collector.stop();
            }
          }
