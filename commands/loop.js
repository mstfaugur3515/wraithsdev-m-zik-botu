    ...
    try {
      ...
      let button = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setCustomId("queue"),
        new ButtonBuilder()
          .setLabel("Şuanki Şarkı")
          .setStyle(ButtonStyle.Secondary)
          .setCustomId("nowplaying"),
        new ButtonBuilder()
          .setLabel("Döngüyü Durdur!")
      ...
      const embed = new EmbedBuilder()
        .setColor('#fc4e03')
        .setAuthor({
        name: 'LMelodilerinizi Döngüye Alın',
        ...
        .setDescription('**Döngü yapıyorum! Müziğin sürekli çalmasına izin verin. **')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Müzik çalmıyor!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `✅ Döngü Sırası!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ Döngü etkinleştirildi!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
