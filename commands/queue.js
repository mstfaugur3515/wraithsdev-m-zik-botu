...
module.exports = {
  name: "queue",
  description: "Size kuyruk listesini gösterir.",
     ...
     const queue = client.player.getQueue(interaction.guild.id);
      ...
      const generateEmbed = async (start) => {
        ...
        return new EmbedBuilder()
          ...
          .setDescription(`▶️ Şimdi dinlediğin: \`${queue.songs[0].name}\`
    ${current.map(data =>
            `\n\`${sayı++}\` | [${data.title}](${data.url}) | (Komut <@${data.user.id}> tarafından kullanıldı)`
          )}`)
          .setFooter({ text: `Sayfa ${page}/${Math.floor(a + 1)}` })
      }
      ...
      await interaction.reply({
        ...
        collector.on("end", async (button) => {
          ...
          const embed = new EmbedBuilder()
            ...
            .setColor(`#ecfc03`)
            .setDescription('▶️ Kuyruk komutunu tekrar yürütün!!')
