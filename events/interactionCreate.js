...
return interaction?.reply({ content: "Rate Limited.", ephemeral: true })
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 Önce Ses kanalına katılın!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `▶️ Yetkin Yok: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`, ephemeral: true });
            }
            } catch (e) {
